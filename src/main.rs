                ...
                for y in 0..grid.size()[0] {
                    ...
                    for x in 0..grid.size()[1] {
                        if let Some(x) = grid.get_item([x, y]).determined_value {
                            s += &match x {
                                Test::Stone => "\u{2588}\u{2588}".color(Color::Grey42).to_string(),
                                Test::Dirt => {
                                    "\u{2588}\u{2588}".color(Color::DarkOrange3a).to_string()
                                }
                                Test::Grass => "\u{2588}\u{2588}".color(Color::Green).to_string(),
                            };
                        } else {
                            s += "  ";
                        }
